...
class TestHovers:
    ...
    def test_hovers_functionality(self, page_manager: PageManager, logger: Logger, actions: ActionChains) -> None:
        ...
        for user_index in range(self.FIRST_USER, self.NUM_OF_USERS+1):
            ...
            assert self.USER + str(user_index) in username_text, f"Expected '{username_text}' to contain '{self.USER + str(user_index)}'"
            ...
            assert self.USERS + str(user_index) in current_url, f"Expected '{current_url}' to contain '{self.USERS + str(user_index)}'"
